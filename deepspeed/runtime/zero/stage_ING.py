...
from deepspeed.runtime.comm.coalesced_collectives import \
    reduce_scatter_coalesced_for_subgroup_order_v2 as reduce_scatter_coalesced_for_subgroup_order
...
class ParoINGOptimizer(ZeROOptimizer):
    """
    ...
    def __init__(self,
        ...
        see_memory_usage("ParoING initialize beginning", force=True)
        ...
        self.partition_count = dist.get_world_size(group=self.dp_process_group)  # 4

        self.zero4_group = groups._get_local_all_to_all_group()

        self.rank = dist.get_rank(group=self.dp_process_group)  # global 0
        self.node_rank = self.rank // get_accelerator().device_count()
        self.num_nodes = self.partition_count // get_accelerator().device_count()
        ...
        self.fp32_partitioned_groups_flat_numel = []
        ...
        self.contiguous_gradients = contiguous_gradients  # True
        ...
        if dist.get_rank(group=self.dp_process_group) == 0:
            see_memory_usage(f"After initializing PaRO-ING optimizer", force=True)
    ...
    def _setup_for_real_optimizer(self):
        see_memory_usage('=' * 50 + 'Before call {} in class {}'.format((inspect.currentframe().f_code.co_name),
                                                                        self.__class__.__name__), force=False)
        ...
        # 1000500, 100w+50
        self.grad_partitions_flat_buffer: Tensor = torch.zeros(sum(p.ds_numel for p in all_params),
                                                               dtype=self.dtype,
                                                               device=self.device)
        ...
        for param in all_params:
            self.__param_id_to_grad_partition[param.ds_id] = self.grad_partitions_flat_buffer.narrow(
                0, offset, param.ds_numel)

            self.mappings[param.ds_id] = self.grad_partitions_flat_buffer.narrow(0, offset,
                                                                                 param.zero4_os_partition_numel())
            offset += param.ds_numel
    ...
    @staticmethod
    def defragment(tensors: List[Tensor], dtype=None) -> Tensor:
        ...
        cpu_buffer = torch.empty(sum(p.numel() for p in tensors),
                                 dtype=dtype or get_only_unique_item(t.dtype for t in tensors),
                                 device="cpu")
    ...
    def _create_fp16_partitions_with_defragmentation(self, fp16_param_groups):
        ...
        for param_group_idx, param_group in enumerate(param_groups):
            for sub_group in param_group:
                ...
                self.fp32_partitioned_groups_flat_numel.append(sum(p.zero4_os_partition_numel() for p in sub_group))
        ...
        if not self.offload_param:  # partitioned params remain in GPU during training
            ...
            fp32_parameter_partitions: List[Tensor] = []
            for sub_group in self.fp16_groups:
                for param in sub_group:
                    parameter_partitions.append(param.ds_tensor)
                    fp32_parameter_partitions.append(param.ds_os_tensor)
            device_buffer = __class__.defragment(parameter_partitions)
            fp32_device_buffer = __class__.defragment(fp32_parameter_partitions, dtype=torch.float).to(self.device)
            ...
            for sub_group in self.fp16_groups:
                ...
                fp32_group_numel = sum(param.zero4_os_partition_numel() for param in sub_group)
                self.fp16_partitioned_groups_flat.append(device_buffer.narrow(0, offset, sub_group_numel))
                self.fp32_partitioned_groups_flat.append(fp32_device_buffer.narrow(0, fp32_offset, fp32_group_numel))
                fp32_offset += fp32_group_numel
    ...
    def _create_fp32_partitions(self):
        ...
        # already create when _create_fp16_partitions_with_defragmentation
    ...
    def initialize_optimizer_states(self):
        ...
        largest_numel = max(self.fp32_partitioned_groups_flat_numel)
        ...
        for i, group in enumerate(self.fp16_groups):
            swappable_optimizer_subgroup = self._swappable_optimizer_subgroup(i)  # False
            swappable_param_subgroup = self.fp16_partitioned_groups_flat[i] is None  # False

            num_elements = int(self.fp32_partitioned_groups_flat_numel[i])
            ...
            if swappable_param_subgroup:  # Fasle
        ...
        for i, param_group in enumerate(self.fp16_groups):
            for param in param_group:
                if param.requires_grad:
                    ...
                    param.partition()
        see_memory_usage('=' * 50 + 'End call {} in class {}'.format((inspect.currentframe().f_code.co_name),
                                                                     self.__class__.__name__), force=False)
    ...
    @property
    def _is_grad_accu_boundary(self):
        return self.micro_step_id == self.gradient_accumulation_steps - 1  # 2
    ...
    def __reduce_and_partition_ipg_grads(self, safe_mode: bool = False) -> None:
        ...
        with get_accelerator().stream(self.reduce_and_partition_stream):
            ...
            if self.contiguous_gradients and not self.reduce_scatter:
                ...
                # (Pdb) [output.shape for output in grad_partitions]
                # [torch.Size([250000]), torch.Size([250]), torch.Size([250000])]

                full_grads_for_rank = self.call_grad_accumulation(self.params_in_ipg_bucket)
                grad_partitions = self.__avg_scatter_grads(full_grads_for_rank)
                self.mapping_param_to_grad_paration(self.params_in_ipg_bucket, grad_partitions)
                # already accumlated
            ...
            if not self.offload_optimizer and self.is_gradient_accumulation_boundary:
                for i, sub_group in enumerate(self.fp16_groups):
                    self.averaged_gradients[i] = [
                        self.mappings[param.ds_id]
                        if param.requires_grad else None for param in sub_group
                    ]
        see_memory_usage('=' * 50 + 'After call {} in class {}'.format((inspect.currentframe().f_code.co_name),
                                                                       self.__class__.__name__), force=False)

    def call_grad_accumulation(self, params_to_reduce):
        """
        我们需要累加完整的梯度。
        """
        see_memory_usage('=' * 50 + 'Before call {} in class {}'.format((inspect.currentframe().f_code.co_name),
        ...
        for param in params_to_reduce:
            grad_partition = param.grad.view(-1)
            ...
            full_grads_for_rank.append(grad_buffer)
            # free the gradient
            param.grad.record_stream(get_accelerator().current_stream())
            param.grad = None
        see_memory_usage('=' * 50 + 'After call {} in class {}'.format((inspect.currentframe().f_code.co_name),
    ...
    @instrument_w_nvtx
    def __avg_scatter_grads(self, full_grads_for_rank) -> List[Tensor]:
        see_memory_usage('=' * 50 + 'Before call {} in class {}'.format((inspect.currentframe().f_code.co_name),
                                                                        self.__class__.__name__), force=False)
        ...
        grad_partitions_for_rank = reduce_scatter_coalesced_for_subgroup_order(full_grads_for_rank,
                                                                               self.dp_process_group,
                                                                               self._is_grad_accu_boundary)
    ...
    def set_grad_positions(self):
        for i, group in enumerate(self.fp16_groups):
            ...
            for param in group:
                param_id = self.get_param_id(param)
                num_elements = param.zero4_os_partition_numel()
                # {0: [0, 0, 250000], 1: [0, 250000, 250], 2: [0, 250250, 250000]}
    ...
    @instrument_w_nvtx
    def mapping_param_to_grad_paration(self, params_to_release: List[Parameter], grad_partitions: List[Tensor]) -> None:
        see_memory_usage('=' * 50 + 'Before call {} in class {}'.format((inspect.currentframe().f_code.co_name),
                                                                        self.__class__.__name__), force=False)
        offload_fp32_gradients = {}
        offload_fp32_offsets = {}
        for param, grad_partition in zip(params_to_release, grad_partitions):
            self.mappings[param.ds_id] = grad_partition
        see_memory_usage('=' * 50 + 'End call {} in class {}'.format((inspect.currentframe().f_code.co_name),
                                                                     self.__class__.__name__), force=False)
    ...
    def _prepare_fp32_grad_for_sub_group(self, sub_group_id):
        ...
        dp_process_id = dist.get_rank(group=self.dp_process_group)
        ...
        assert single_grad_partition.numel() == self.fp32_partitioned_groups_flat[sub_group_id].numel(), \
            "averaged gradients have different number of elements that partition size {} {} {} {}".format(
                single_grad_partition.numel(), self.fp32_partitioned_groups_flat[sub_group_id].numel(), sub_group_id,
                dp_process_id)
    ...
    def gather_params_sharded_by_optimizer(self, subgroup_id, group):
        if self.num_nodes > 1:
            log_dist(f'Begin broadcast params, subgroup_id: {subgroup_id}', ranks=[-1], level=logging.DEBUG)
            dtype = self.fp16_partitioned_groups_flat[0].dtype
            device = self.fp32_partitioned_groups_flat[subgroup_id].device
            partition_numel = self.fp32_partitioned_groups_flat[subgroup_id].numel()
            reduce_buffer = torch.zeros(self.num_nodes * partition_numel,
                                        dtype=dtype,
                                        device=device).flatten()
            my_rank = self.node_rank
            gathers = [
                reduce_buffer.narrow(0, partition_numel * i, partition_numel) for i in range(self.num_nodes)
            ]
            gathers[my_rank].data.copy_(self.fp32_partitioned_groups_flat[subgroup_id].data, non_blocking=False)

            dist.all_gather(gathers, gathers[my_rank],
                            group=self.zero4_group[f'global_{get_accelerator().current_device()}'])
        offset = 0
        for p in group:
            os_partition_size = p.zero4_os_partition_numel()
            if self.num_nodes <= 1:
                p.ds_tensor.data.copy_(
                    self.fp32_partitioned_groups_flat[subgroup_id].narrow(0, offset, os_partition_size))
            else:
                for node_idx in range(self.num_nodes):
                    p.ds_tensor.narrow(0, node_idx * os_partition_size, os_partition_size).data.copy_(
                        gathers[node_idx].narrow(0, offset, os_partition_size))
            offset += os_partition_size
    ...
    def step(self, closure=None):
        ...
        for sub_group_id, group in enumerate(self.fp16_groups):
            ...
            # gather params from other nodes
            self.gather_params_sharded_by_optimizer(sub_group_id, group)
    ...
    def has_overflow(self, partition_gradients=True):
        if partition_gradients:
            ...
            dist.all_reduce(overflow_gpu, op=dist.ReduceOp.MAX, group=self.zero4_group[f'local_{self.node_rank}'])
    ...
    def _rigid_state_dict(self):
        ...
        state_dict[GROUP_PARTITION_COUNT] = get_accelerator().device_count()
